...
export function Sidebar({ userEmail }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <header className="md:hidden fixed top-0 left-0 right-0 bg-slate-900 border-b border-slate-700/60 flex items-center justify-between px-4 py-3 z-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white text-base tracking-tight">GuardSport</span>
        </div>
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-60 flex-shrink-0 bg-slate-900 border-r border-slate-700/60 flex-col min-h-screen sticky top-0">
      {/* Logo */}
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-700/60 flex justify-around items-center px-2 py-2 z-50 pb-[env(safe-area-inset-bottom)]">
        {navItems.map(item => {
          const isActive = pathname === item.href
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center p-2 rounded-lg text-[10px] font-medium transition-colors duration-150 ${
                isActive
                  ? 'text-indigo-400'
