...
export default async function DashboardPage() {
  ...
  const [{ data: assets }, { data: violations }] = await Promise.all([
    supabase.from('assets').select('id'),
    supabase.from('violations').select('id, status, detected_at, found_url, platform, assets(title)'),
  ])
  ...
  return (
    <div>
      ...
      {/* Chart */}
      <Card title="Violations — Last 7 Days" className="mb-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700/60">
                  ...
                  <th className="text-left py-2.5 px-2 text-xs uppercase tracking-wide text-slate-400 font-medium">Date</th>
                </tr>
              </thead>
              <tbody>
